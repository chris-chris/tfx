...
from tfx.components import model_validator
  ...
  def resolve(
    ...
    all_models = sorted(
        metadata_handler.get_published_artifacts_by_type_within_context(
            source_channels[model_channel_key].type_name, pipeline_context.id),
        key=lambda m: m.id,
        reverse=True)
    # Gets all ModelBlessing artifacts in the search space.
    all_model_blessings = metadata_handler.get_published_artifacts_by_type_within_context(
